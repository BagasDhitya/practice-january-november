...
export default function TodoPlaceholder() {
    const { todosPlaceholder, loading, error, setClick } = usePlaceholder()

    return (
        <div className="w-screen h-full flex flex-col items-center justify-center">
            {loading !== true ? <div className="grid grid-cols-3 gap-3 p-5">
                {todosPlaceholder.length > 0 ?
                    ...
                    <button className="p-5 rounded-md bg-blue-500 text-white" onClick={() => setClick("Load Data")}>Load Data</button>
                }
            </div> :
